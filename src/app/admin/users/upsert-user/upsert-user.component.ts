...
@Component({
  ...
  imports: [CommonModule, FormsModule, ReactiveFormsModule, MatFormFieldModule, MatInputModule, MatSelectModule, MatGridListModule, MatButtonModule, MatDialogModule, MatProgressSpinnerModule],
  providers: [
    {
...
export class UpsertUserComponent implements OnInit {
  constructor(public dialogRef: MatDialogRef<{ type: "create" | "update" } & User>, @Inject(MAT_DIALOG_DATA) public data: User, private userService: UserService) {}
  ...
  handleFormSubmit() {
    ...
    if (!isUpdate) {
      this.userService.createUser({ ...(this.userFormGroup.value as User) }).subscribe((res) => {
    ...
    if (isUpdate) {
      this.userService.updateUser({ ...(this.userFormGroup.value as User), id: this.data.id }).subscribe(() => {
        this.isLoading = false;
        this.dialogRef.close({
          type: "update",
